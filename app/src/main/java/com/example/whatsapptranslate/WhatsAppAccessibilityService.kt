...
class WhatsAppAccessibilityService : AccessibilityService() {
    ...
    override fun onServiceConnected() {
        if (!isAccessibilityServiceEnabled()) {
            Toast.makeText(this, "Por favor, habilita el servicio de accesibilidad para usar la aplicación", Toast.LENGTH_LONG).show()
            return
        }
    ...
    // Añade esta función a tu clase
    private fun isAccessibilityServiceEnabled(): Boolean {
        val accessibilityManager = getSystemService(Context.ACCESSIBILITY_SERVICE) as AccessibilityManager
        val enabledServices = accessibilityManager.getEnabledAccessibilityServiceList(AccessibilityServiceInfo.FEEDBACK_ALL_MASK)
        return enabledServices.any { it.id == "$packageName/.WhatsAppAccessibilityService" }
    }
}
